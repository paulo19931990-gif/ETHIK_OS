...
async function salvarDocumento(silencioso = false) {
    ...
    try {
        await aprenderPecasDaOS(); // INTELIGÊNCIA: Aprende as peças novas antes de salvar
...
// --- SISTEMA INTELIGENTE DE PEÇAS E AUTOCOMPLETAR ---
let bancoPecas = [];
...
async function iniciarBancoPecas() {
    ...
    if (!salvo || salvo.length === 0) {
        await localforage.setItem('banco_pecas_inteligente', pecasIniciais);
        bancoPecas = pecasIniciais;
    } else {
        bancoPecas = salvo;
    }
    atualizarListasHTML();
...
function atualizarListasHTML() {
    const dlCodigos = document.getElementById('dbCodigosPecas');
...
function autoPreencherPeca(input, tipo) {
    const row = input.closest('.peca-row-item');
    const inputNome = row.querySelector('.n');
    const inputCod = row.querySelector('.c');
    
    if (tipo === 'codigo' && input.value) {
        const pecaEncontrada = bancoPecas.find(p => p.c === input.value);
        if (pecaEncontrada && !inputNome.value) inputNome.value = pecaEncontrada.n;
    } else if (tipo === 'nome' && input.value) {
        const pecaEncontrada = bancoPecas.find(p => p.n === input.value);
        if (pecaEncontrada && !inputCod.value) inputCod.value = pecaEncontrada.c;
    }
}
...
async function aprenderPecasDaOS() {
    ...
    document.querySelectorAll('.peca-row-item').forEach(row => {
        let n = row.querySelector('.n').value.trim();
        let c = row.querySelector('.c').value.trim();
        
        if (n && c) {
            const existe = bancoPecas.find(p => p.c === c || p.n === n);
            if (!existe) {
                bancoPecas.push({ c: c, n: n });
    ...
    if (bancoAtualizado) {
        await localforage.setItem('banco_pecas_inteligente', bancoPecas);
        atualizarListasHTML();
    }
}
